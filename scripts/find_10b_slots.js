...
async function findSlots() {
  ...
  const l10B = await prisma.lesson.findMany({
    where: { schoolId, scheduleId: activeSchedId, class: { name: '10-B' } },
  ...
  console.log('=== 10-B VA SABOHAT BO\'SH VAQTLARI ===');
  for (let d = 1; d <= 6; d++) {
    const bDay = l10B.filter(l => l.dayOfWeek === d);
    const sDay = saboL.filter(l => l.dayOfWeek === d && !(l.class.name === '9-A' && l.groupType === 'GROUP_2'));
    
    console.log(`\nKun ${d}:`);
    for (let p = 1; p <= 7; p++) {
      const bLesson = bDay.find(l => l.periodNumber === p);
      const sLesson = sDay.find(l => l.periodNumber === p);
      const bStatus = bLesson ? `${bLesson.subject.name}` : 'BO\'SH';
      const sStatus = sLesson ? `${sLesson.class.name} (${sLesson.subject.name})` : 'BO\'SH';
      
      const bothFree = (!bLesson && !sLesson) ? ' <--- IKKISI HAM BO\'SH!' : '';
      console.log(`  P${p}: 10-B=[${bStatus}] | Sabohat=[${sStatus}]${bothFree}`);
    }
  }
}

findSlots().catch(console.error).finally(() => prisma.$disconnect());
